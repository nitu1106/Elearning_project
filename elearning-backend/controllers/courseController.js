...
const updateCourse = asyncHandler(async (req, res) => {
  ...
  if (updates.requirements)
    updates.requirements = safeParse(updates.requirements);
      ...
      for (const lec of mod.lectures) {
        if (lec.publicId)
          await cloudinary.uploader
            .destroy(lec.publicId, { resource_type: "video" })
            .catch(() => {});
      }
    }
  }
...
const addModule = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json({ success: true, message: "Module added.", data: course.modules });
});
...
const uploadMaterial = asyncHandler(async (req, res) => {
  ...
  // Handle JSON body with URL (from disk upload endpoint)
  // ✅ Fix — use req.body values in the JSON block
if (req.body.url && req.body.name) {
  materialData = {
    name:     req.body.name,   // ← use body, not file
    url:      req.body.url,    // ← use body, not file
    fileType: req.body.fileType || 'application/octet-stream',
  };
}
  // Handle direct file upload (Cloudinary or multer)
  else if (req.file) {
    materialData = {
  ...
  lecture.materials.push(materialData);
  await course.save();
...
const getInstructorCourses = asyncHandler(async (req, res) => {
  const courses = await Course.find({ instructor: req.user._id })
    .select(
      "title status enrollmentCount rating createdAt thumbnail totalLectures",
    )
    .sort({ createdAt: -1 });
