...
import API from "../../api"; // ← make sure your default axios instance is imported
import {
  BookOpen,
  Plus,
  Users,
      ...
      <div style={{ maxWidth: 700 }}>
        ...
        >
          ...
          <div className="form-group">
            ...
            <input
              className="form-input"
              placeholder="Basic HTML, Computer with internet"
...
export const ManageCoursePage = () => {
  ...
  // ── Material uploading state (tracks which lecture is uploading) ────────────
  const [uploadingLecId, setUploadingLecId] = useState(null);
  ...
  const saveModuleEdit = async (modId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/courses/${id}/modules/${modId}`,
        {
          method: "PATCH",
          headers: {
  ...
  // ── Upload material ─────────────────────────────────────────────────────────
  // FIX: Two-step upload:
  //   1. POST file to /api/upload  → get back { url, name }
  //   2. POST { url, name } as JSON to /api/courses/.../materials
  const handleMaterialUpload = async (file, modId, lecId) => {
    setUploadingLecId(lecId);
    try {
      // Step 1 — upload file to server, get a real URL back
      const fd = new FormData();
      fd.append("file", file);
      const uploadRes = await API.post("/upload", fd);
      const { url, name } = uploadRes.data;

      // Step 2 — save URL + name to the lecture's materials array
      await courseAPI.uploadMaterial(id, modId, lecId, { url, name });
        ...
        <div className="card" key={mod._id || mi} style={{ marginBottom: 16 }}>
          ...
          >
            {editModId === mod._id ? (
              ...
              <div
                style={{ display: "flex", gap: 8, flex: 1, marginRight: 12 }}
              >
                <input
                  className="form-input"
                  value={editModTitle}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") saveModuleEdit(mod._id);
                    if (e.key === "Escape") setEditModId(null);
                  }}
                  autoFocus
                  style={{ flex: 1 }}
                />
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => saveModuleEdit(mod._id)}
                >
                  Save
                </button>
                <button
          ...
          {mod.lectures?.length === 0 && (
            <p
              style={{ color: "var(--text3)", fontSize: 13, padding: "8px 0" }}
            >
            ...
            >
              {editLec?.lec._id === lec._id ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {/* ── Material upload — FIXED ── */}
                  <label style={{ cursor: uploadingLecId === lec._id ? "wait" : "pointer" }}>
                    <input
                      type="file"
                      accept=".pdf,.doc,.docx,.ppt,.pptx"
                      style={{ display: "none" }}
                      disabled={uploadingLecId === lec._id}
                      onChange={async (e) => {
                        const file = e.target.files[0];
                        if (!file) return;
                        await handleMaterialUpload(file, mod._id, lec._id);
                        e.target.value = "";
                      }}
                    />
                    <span
                      style={{
                        ...
                        alignItems: "center",
                        gap: 4,
                        opacity: uploadingLecId === lec._id ? 0.6 : 1,
                      }}
                    >
                      {uploadingLecId === lec._id ? "Uploading…" : "+ Material"}
                    </span>
                  </label>
...
export const InstructorQuizzesPage = () => {
  ...
  const updateQText = (qi, val) => {
    const q = [...questions];
    q[qi].questionText = val;
    setQuestions(q);
  };
  const updateQMarks = (qi, val) => {
    const q = [...questions];
    q[qi].marks = Number(val);
    setQuestions(q);
  };
  const updateOptTxt = (qi, oi, val) => {
    const q = [...questions];
  ...
  const setCorrect = (qi, oi) => {
    const q = [...questions];
    q[qi].options = q[qi].options.map((o, i) => ({
      ...o,
      isCorrect: i === oi,
    }));
    setQuestions(q);
  ...
  const createQuiz = async () => {
    ...
    for (let i = 0; i < questions.length; i++) {
      if (!questions[i].questionText.trim()) {
        toast.error(`Q${i + 1}: Enter question text`);
        return;
      }
      if (!questions[i].options.every((o) => o.text.trim())) {
        toast.error(`Q${i + 1}: Fill all 4 options`);
        return;
      }
      if (!questions[i].options.some((o) => o.isCorrect)) {
        toast.error(`Q${i + 1}: Mark the correct answer`);
        return;
      }
    }
    try {
              ...
              >
                ...
                {q.options.map((opt, oi) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={opt.text}
                      onChange={(e) => updateOptTxt(qi, oi, e.target.value)}
                      style={{
                        flex: 1,
                        borderColor: opt.isCorrect ? "#10b981" : undefined,
                      }}
                    />
                    {opt.isCorrect && (
                      <span
                ...
                >
                  ...
                  <input
                    ...
                    min="1"
                    value={q.marks}
                    onChange={(e) => updateQMarks(qi, e.target.value)}
                    style={{ width: 70 }}
                  />
                </div>
              </div>
